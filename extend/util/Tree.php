...
class Tree{  
    ...
    private static function makeTreeCore($index,$data,$type='linear')
    {
        ...
        {
            if($type=='normal'){
                $val=$primary_index?$data[$item[$primary_key]]:$data[$id];
                if(isset($val))
                {
                ...
                $index==0||self::$result[$parent_id]['sonid'].=$item[$primary_key].',';
                $item['level'] = self::$level[$id];
                if($primary_index){
                    self::$result[$item[$primary_key]] = $item;
                }else{
                    self::$result[] = $item;
                }
                if(isset($data[$id])){
